...
// When responding to a method, we need to return a GVariant value wrapped in a tuple. This method will simplify this
// slightly by wrapping a GVariant of the type "ay" and wrapping it in a tuple before sending it off as the method
// response.
//
// This is the generalized form that accepts a GVariant *. There is a templated helper method (`methodReturnValue()`)
// that accepts common types.
void GattInterface::methodReturnVariant(GDBusMethodInvocation *pInvocation, GVariant *pVariant, bool wrapInTuple)
    const {
    if (wrapInTuple) {
