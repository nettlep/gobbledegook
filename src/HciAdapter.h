...
public:
    ...
    struct CommandCompleteEvent {
        ...
        std::string debugText() {
            ...
            text += "  + Command code       : " + Utils::hex(commandCode) + " (" +
                    HciAdapter::kCommandCodeNames[commandCode] + ")\n";
    ...
    struct DeviceConnectedEvent {
        ...
        std::string debugText() {
            ...
            if (eirDataLength > 0) {
                text += "\n";
                text += "  + EIR Data           : " +
                        Utils::hex(reinterpret_cast<uint8_t *>(&eirDataLength) + 2, eirDataLength);
            }
            return text;
        }
    } __attribute__((packed));
    ...
    struct AdapterSettings {
        ...
        std::string toString() {
            std::string text = "";
            if (isSet(EHciPowered)) {
                text += "Powered, ";
            }
            if (isSet(EHciConnectable)) {
                text += "Connectable, ";
            }
            if (isSet(EHciFastConnectable)) {
                text += "FC, ";
            }
            if (isSet(EHciDiscoverable)) {
                text += "Discov, ";
            }
            if (isSet(EHciBondable)) {
                text += "Bondable, ";
            }
            if (isSet(EHciLinkLevelSecurity)) {
                text += "LLS, ";
            }
            if (isSet(EHciSecureSimplePairing)) {
                text += "SSP, ";
            }
            if (isSet(EHciBasicRate_EnhancedDataRate)) {
                text += "BR/EDR, ";
            }
            if (isSet(EHciHighSpeed)) {
                text += "HS, ";
            }
            if (isSet(EHciLowEnergy)) {
                text += "LE, ";
            }
            if (isSet(EHciAdvertising)) {
                text += "Adv, ";
            }
            if (isSet(EHciSecureConnections)) {
                text += "SC, ";
            }
            if (isSet(EHciDebugKeys)) {
                text += "DebugKeys, ";
            }
            if (isSet(EHciPrivacy)) {
                text += "Privacy, ";
            }
            if (isSet(EHciControllerConfiguration)) {
                text += "ControllerConfig, ";
            }
            if (isSet(EHciStaticAddress)) {
                text += "StaticAddr, ";
            }
    ...
    // The comments documenting these fields are very high level. There is a lot of detailed information not present,
    // for example some values are not available at all times. This is fully documented in:
    //
...
private:
    ...
    HciAdapter() :
        commandResponseLock(commandResponseMutex),
        activeConnections(0) {}
