...
//
// Methods are identified by their name (such as "Get" or "Set"). They have argument definitions (defined as part of
...
struct DBusMethod {
    ...
    // Calls the method
    //
    // If a callback delegate has been set, then this method will call that delegate, otherwise this method will do
    // nothing
    template <typename T>
    void call(
        GDBusConnection *pConnection,
        ...
        if (!callback) {
            Logger::error(
                SSTR << "DBusMethod contains no callback: [" << path << "]:[" << interfaceName << "]:[" << methodName
                     << "]"
            );
