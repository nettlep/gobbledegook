...
//
// This file contains raw communication layer for the Bluetooth Management API, which is used to configure the Bluetooth
// adapter (such as enabling LE, setting the device name, etc.) This class is used by HciAdapter (HciAdapter.h) to
...
//
// Returns true if data was read successfully, otherwise false is returned. A false return code does not necessarily
// depict an error, as this can arise from expected conditions (such as an interrupt.)
...
void HciSocket::logErrno(const char *pOperation) const {
    ...
    Logger::error(
        SSTR << "Error on Bluetooth management socket during " << pOperation << " operation. Error code " << errno
             << ": " << errorDetail
    );
}
