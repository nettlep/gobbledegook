...
// >>
//
// This file contains helper function declarations for our server's implementation. That is, methods that are used by
// the server itself (when responding to D-Bus or BlueZ requests.)
//
// >>
// >>>  DISCUSSION
// >>
...
struct ServerUtils {
    ...
    //
    // This routine will attempt to parse /proc/cpuinfo to return the CPU count/model. Results are cached on the first
    // call, with cached results returned on successive calls.
    //
