...
// >>
//
// This is our Logger, which allows for applications to use their own logging mechanisms by registering log receivers
// for each of the logging categories.
//
// >>
// >>>  DISCUSSION
// >>
//
// In order for logging to work, the application must register logging receivers with the `ggkLogRegister*` methods. At
// that point, it is up to the application to decide how to handle the incoming log requests. For example, it may filter
// Error and Fatal logs to stderr.
//
// The following is a breakdown of the intended use of each of the available categories. Application receivers can
// filter then (enable/ disable/redirect/etc.) in whatever way works best (via bitmasks, log levels, etc.)
//
//     Debug  - Lowest logging level; may contain log-spam
//     Info   - The logging equivalent to the junk drawer
//     Status - Current health/service availability info (ex: "Server starting", "Initialization complete", "Stop
//     requested") Warn   - Problems that may or may not affect functionality Error  - Problems that will affect
//     functionality, but do not result in a termination of the process Fatal  - Issues that so severe as to require
//     termination
//
...
//     Trace  - Useful for tracing through code (ex: "Before call to foo()", "Entering function bar") and other
//     development use
...
// Register logging receiver for DEBUG logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerDebugReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverDebug = receiver;
}

// Register logging receiver for INFO logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerInfoReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverInfo = receiver;
}

// Register logging receiver for STATUS logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerStatusReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverStatus = receiver;
}

// Register logging receiver for WARN logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerWarnReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverWarn = receiver;
}

// Register logging receiver for ERROR logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerErrorReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverError = receiver;
}

// Register logging receiver for FATAL logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerFatalReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverFatal = receiver;
}

// Register logging receiver for ALWAYS logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerAlwaysReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverAlways = receiver;
}

// Register logging receiver for TRACE logging.  To register a logging level, simply call with a delegate that performs
// the appropriate logging action. To unregister, call with `nullptr`
void Logger::registerTraceReceiver(GGKLogReceiver receiver) {
    Logger::logReceiverTrace = receiver;
}
...
void Logger::debug(const char *pText) {
    if (nullptr != Logger::logReceiverDebug) {
        Logger::logReceiverDebug(pText);
    }
}
...
void Logger::info(const char *pText) {
    if (nullptr != Logger::logReceiverInfo) {
        Logger::logReceiverInfo(pText);
    }
}
...
void Logger::status(const char *pText) {
    if (nullptr != Logger::logReceiverStatus) {
...
void Logger::warn(const char *pText) {
    if (nullptr != Logger::logReceiverWarn) {
        Logger::logReceiverWarn(pText);
    }
}
...
void Logger::error(const char *pText) {
    if (nullptr != Logger::logReceiverError) {
        Logger::logReceiverError(pText);
    }
}
...
void Logger::fatal(const char *pText) {
    if (nullptr != Logger::logReceiverFatal) {
        Logger::logReceiverFatal(pText);
    }
}
...
void Logger::always(const char *pText) {
    if (nullptr != Logger::logReceiverAlways) {
        Logger::logReceiverAlways(pText);
    }
}
...
void Logger::trace(const char *pText) {
    if (nullptr != Logger::logReceiverTrace) {
        Logger::logReceiverTrace(pText);
    }
}
