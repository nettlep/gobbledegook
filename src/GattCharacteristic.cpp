...
// >>
//
// A GATT characteristic is the component within the Bluetooth LE standard that holds and serves data over Bluetooth.
// This class is intended to be used within the server description. For an explanation of how this class is used, see
// the detailed discussion in Server.cpp.
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
bool GattCharacteristic::callMethod(
    ...
    for (const DBusMethod &method : methods) {
        if (methodName == method.getName()) {
            method.call<
                GattCharacteristic>(pConnection, getPath(), getName(), methodName, pParameters, pInvocation, pUserData);
            return true;
        }
    }
...
// TickEvent::Callback type. We also return our own type. This simplifies the server description by allowing call to
// chain.
...
// server, then it may be beneficial to call this method from within your onWriteValue callback to reduce duplicated
// code. See `callOnUpdatedValue` for more information.
...
//
// If you need to perform the same action(s) when a value is updated from the client (via onWriteValue) or from this
// server, then it may be beneficial to place those actions in the `onUpdatedValue` method and call it from from within
// your `onWriteValue` callback to reduce duplicated code. To call the `onUpdatedValue` method from within your
// `onWriteValue`, you can use this pattern:
//
...
GattDescriptor &GattCharacteristic::gattDescriptorBegin(
    ...
    GattDescriptor &descriptor =
        *child.addInterface(std::make_shared<GattDescriptor>(child, *this, "org.bluez.GattDescriptor1"));
...
//
// This is a generalized method that accepts a `GVariant *`. A templated version is available that supports common types
// called `sendChangeNotificationValue()`.
//
// The caller may choose to consult HciAdapter::getInstance().getActiveConnectionCount() in order to determine if there
// are any active connections before sending a change notification.
