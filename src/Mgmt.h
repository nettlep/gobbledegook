...
struct Mgmt {
    ...
    //
    // Set `controllerIndex` to the zero-based index of the device as recognized by the OS. If this parameter is
    // omitted, the index of the first device (0) will be used.
    Mgmt(uint16_t controllerIndex = kDefaultControllerIndex);

    // Set the adapter name and short name
    //
    // The inputs `name` and `shortName` may be truncated prior to setting them on the adapter. To ensure that `name`
    // and `shortName` conform to length specifications prior to calling this method, see the constants
    // `kMaxAdvertisingNameLength` and `kMaxAdvertisingShortNameLength`. In addition, the static methods
    // `truncateName()` and `truncateShortName()` may be helpful.
    //
    ...
    bool addAdvertising(std::string shortName, const uint8_t *uuid);

    //
    // Utilitarian
    //

    // Truncates the string `name` to the maximum allowed length for an adapter name. If `name` needs no truncation, a
    // copy of `name` is returned.
    static std::string truncateName(const std::string &name);

    // Truncates the string `name` to the maximum allowed length for an adapter short-name. If `name` needs no
    // truncation, a copy of `name` is returned.
