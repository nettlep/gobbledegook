...
Server::Server(
    ...
    objects
        ...
        //
        // We also handle updates to the battery level from inside the server (see onUpdatedValue). There is an external
        // method (see main.cpp) that updates our battery level and posts an update using ggkPushUpdateQueue. Those
        // updates are used to notify us that our value has changed, which translates into a call to `onUpdatedValue`
        // from the idleFunc (see Init.cpp). .gattServiceBegin("battery", "180F")
        ...
        // 		//
        // 		// Here we use the onUpdatedValue to set a callback that isn't exposed to BlueZ, but rather
        // allows us to manage
        // 		// updates to our value. These updates may have come from our own server or some other source.
        // 		//
        // 		// We can handle updates in any way we wish, but the most common use is to send a change
        // notification. 		.onUpdatedValue(CHARACTERISTIC_UPDATED_VALUE_CALLBACK_LAMBDA
        // 		{
        // 			uint8_t batteryLevel = self.getDataValue<uint8_t>("battery/level", 0);
        ...
        //
        // This showcases the use of events (see the call to .onEvent() below) for periodic actions. In this case, the
        // action taken is to update time every tick. This probably isn't a good idea for a production service, but it
        // has been quite useful for testing to ensure we're connected and updating. .gattServiceBegin("time", "1805")
        ...
        //
        // This service will return a text string value (default: 'Hello, world!'). If the text value is updated, it
        ...
        // // 	.onReadValue(DESCRIPTOR_METHOD_CALLBACK_LAMBDA
        // // 	{
        // // 		const char *pDescription = "A mutable text string used for testing. Read and write to me, it
        // // tickles!"; 		self.methodReturnValue(pInvocation, pDescription, true);
        // // 	})
    ...
    //
    // This service will simply return the result of asctime() of the current local time. It's a nice test service to
    // provide a new value each time it is read.
    ...
    // 			.onReadValue(DESCRIPTOR_METHOD_CALLBACK_LAMBDA
    // 			{
    // 				const char *pDescription = "Returns the local time (as reported by POSIX asctime()) each
    // time it is read"; 				self.methodReturnValue(pInvocation, pDescription, true);
    // 			})
