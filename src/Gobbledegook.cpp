...
// >>
//
// Although Gobbledegook requires customization (see Server.cpp), it still maintains a public interface (this file.)
// This keeps the interface to the server simple and compact, while also allowing the server to be built as a library if
// the developer so chooses.
//
// As an alternative, it is also possible to publish customized Gobbledegook servers, allowing others to use the server
// through the public interface defined in this file.
//
// In addition, this interface is compatible with the C language, allowing non-C++ programs to interface with
// Gobbledegook, even though Gobbledgook is a C++ codebase. This should also simplify the use of this interface with
...
void setServerRunState(GGKServerRunState newState) {
    Logger::status(
        SSTR << "** SERVER RUN STATE CHANGED: " << ggkGetServerRunStateString(serverRunState) << " -> "
             << ggkGetServerRunStateString(newState)
    );
...
void setServerHealth(GGKServerHealth newHealth) {
    Logger::status(
        SSTR << "** SERVER HEALTH CHANGED: " << ggkGetServerHealthString(serverHealth) << " -> "
             << ggkGetServerHealthString(newHealth)
    );
...
// ---------------------------------------------------------------------------------------------------------------------------------
//  _   _           _       _ _
// | | | |_ __   __| | __ _| |_ ___     __ _ _   _  ___ _   _  ___    _ __ ___   __ _ _ __   __ _  __ _  ___ _ __ ___
// ___ _ __ | |_ | | | | '_ \ / _` |/ _` | __/ _ \   / _` | | | |/ _ \ | | |/ _ \  | '_ ` _ \ / _` | '_ \ / _` |/ _` |/
// _ \ '_ ` _ \ / _ \ '_ \| __| | |_| | |_) | (_| | (_| | ||  __/  | (_| | |_| |  __/ |_| |  __/  | | | | | | (_| | | |
// | (_| | (_| |  __/ | | | | |  __/ | | | |_
//  \___/| .__/ \__,_|\__,_|\__\___|   \__, |\__,_|\___|\__,_|\___|  |_| |_| |_|\__,_|_| |_|\__,_|\__, |\___|_| |_|
//  |_|\___|_| |_|\__|
//       |_|                              |_|                                                     |___/
//
// Push/pop update notifications onto a queue. As these methods are where threads collide (i.e., this is how they
// communicate), these methods are thread-safe.
// ---------------------------------------------------------------------------------------------------------------------------------
...
int ggkPopUpdateQueue(char *pElementBuffer, int elementLen, int keep) {
...

    {
        ...
        if (result.length() + 1 > static_cast<size_t>(elementLen)) {
            return -1;
        }
...
int ggkWait() {
    ...
    try {
        ...
        if (ex.code() == std::errc::invalid_argument) {
            ...
            Logger::warn(
                SSTR << "Deadlock avoided in call to ggkWait() (did the server thread try to stop itself?): "
                     << ex.what()
            );
        } else {
...
int ggkStart(
    ...
    try {
        ...
        printHandlerGLib = g_set_print_handler([](const gchar *string) { Logger::info(string); });
        printerrHandlerGLib = g_set_printerr_handler([](const gchar *string) { Logger::error(string); });
        logHandlerGLib = g_log_set_default_handler(
            [](const gchar *log_domain, GLogLevelFlags log_levels, const gchar *message, gpointer /*user_data*/) {
        ...
        try {
            ...
            Logger::error(
                SSTR << "Server thread was unable to start (code " << ex.code() << ") during ggkStart(): " << ex.what()
            );
