...
// >>
//
// This file contains various general utilitarian functions used throught. It is in some ways, the 'junk drawer' of the
// appliation, though better organized than most physical junk drawers.
//
// >>
// >>>  DISCUSSION
// >>
...
struct Utils {
    ...
    //
    // USE WITH CAUTION: It is expected that pAddress point to an array of 6 bytes. The length of the array cannot be
    // validated and incorrect lengths will produce undefined, likely unwanted and potentially fatal results. Or it will
    // return the address of the train at platform 9 3/4. You decide.
    //
    ...
    // -----------------------------------------------------------------------------------------------------------------------------
    // A small collection of helper functions for generating various types of GVariants, which are needed when
    // responding to BlueZ method/property messages. Real services will likley need more of these to support various
    // types of data passed to/from BlueZ, or feel free to do away with them and use GLib directly.
    // -----------------------------------------------------------------------------------------------------------------------------
    ...
    //
    // This is an extension method to the vararg version, which accepts pass-through variable arguments from other
    // mthods.
    ...
    // Endian conversion
    //
    // The Bluetooth Management API defines itself has using little-endian byte order. In the methods below, 'Hci'
    // refers to this format, while 'Host' refers to the endianness of the hardware we are running on.
    //
    // The `Utils::endianToHost()` method overloads perform endian byte-ordering conversions from the HCI to our endian
    // format The `Utils::endianToHci()` method overloads perform endian byte-ordering conversions from our endian
    // format to that of the HCI
    // -----------------------------------------------------------------------------------------------------------------------------
    ...
    static uint8_t endianToHost(uint8_t value) {
        return value;
    }
