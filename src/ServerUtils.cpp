...
// >>
//
// This file contains helper functions for our server's implementation. That is, methods that are used by the server
...
// >>
//
// Generally speaking, these are blocks of code that are too big to comfortably fit as lambdas within the
// `Server::Server()` constructor are here.
//
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
// Adds an object to the tree of managed objects as returned from the `GetManagedObjects` method call from the D-Bus
...
//     (a{oa{sa{sv}}})
static void
addManagedObjectsNode(const DBusObject &object, const DBusObjectPath &basePath, GVariantBuilder *pObjectArray) {
    ...
    if (!object.getInterfaces().empty()) {
        ...
        for (std::shared_ptr<const DBusInterface> pInterface : object.getInterfaces()) {
            ...
            if (std::shared_ptr<const GattService> pService =
                    TRY_GET_CONST_INTERFACE_OF_TYPE(pInterface, GattService)) {
                if (!pService->getProperties().empty()) {
                    ...
                    for (const GattProperty &property : pService->getProperties()) {
                        ...
                        g_variant_builder_add(pPropertyArray, "{sv}", property.getName().c_str(), property.getValue());
                    }

                    g_variant_builder_add(pInterfaceArray, "{sa{sv}}", pService->getName().c_str(), pPropertyArray);
                }
                ...
                if (!pDescriptor->getProperties().empty()) {
                    ...
                    g_variant_builder_add(pInterfaceArray, "{sa{sv}}", pDescriptor->getName().c_str(), pPropertyArray);
                }
            } else {
        ...
        g_variant_builder_add(pObjectArray, "{oa{sa{sv}}}", path.c_str(), pInterfaceArray);
    }
...
// Builds the response to the method call `GetManagedObjects` from the D-Bus interface
...
//
// This routine will attempt to parse /proc/cpuinfo to return the CPU count/model. Results are cached on the first call,
// with cached results returned on successive calls.
//
...
std::string ServerUtils::getCpuInfo(int16_t &cpuCount) {
    ...
    if (cachedCount == -1) {
        ...
        if (cachedModel.empty()) {
            cachedModel = "Gooberfest Cyclemaster 3000 (v8)";
        }
        if (cachedCount == 0) {
            cachedCount = 42;
        }
