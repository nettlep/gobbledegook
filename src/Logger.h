...
public:
    ...
    // Register logging receiver for DEBUG logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerDebugReceiver(GGKLogReceiver receiver);

    // Register logging receiver for INFO logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerInfoReceiver(GGKLogReceiver receiver);

    // Register logging receiver for STATUS logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerStatusReceiver(GGKLogReceiver receiver);

    // Register logging receiver for WARN logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerWarnReceiver(GGKLogReceiver receiver);

    // Register logging receiver for ERROR logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerErrorReceiver(GGKLogReceiver receiver);

    // Register logging receiver for FATAL logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerFatalReceiver(GGKLogReceiver receiver);

    // Register logging receiver for ALWAYS logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
    static void registerAlwaysReceiver(GGKLogReceiver receiver);

    // Register logging receiver for TRACE logging.  To register a logging level, simply call with a delegate that
    // performs the appropriate logging action. To unregister, call with `nullptr`
