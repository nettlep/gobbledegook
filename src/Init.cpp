...
//    Graceful shutdown
//
// Want to poke around and see how things work? Here's a tip: Start at the bottom of the file and work upwards. It'll
...
//                                                  |_|                                        |___/
//
// Our idle funciton is what processes data updates. We handle this in a simple way. We update the data directly in our
// global `TheServer` object, then call `ggkPushUpdateQueue` to trigger that data to be updated (in whatever way the
// service responsible for that data() sees fit.
//
// This is done using the `ggkPushUpdateQueue` / `ggkPopUpdateQueue` methods to manage the queue of pending update
// messages. Each entry represents an interface that needs to be updated. The idleFunc calls the interface's
// `onUpdatedValue` method for each update.
//
// The idle processor will perform one update per idle tick, however, it will notify that there is more data so the idle
// ticks do not lag behind.
// ---------------------------------------------------------------------------------------------------------------------------------

// Our idle function
//
// This method is used to process data on the same thread as our main loop. This allows us to communicate with our
// service from the outside.
//
// IMPORTANT: This method must return 'true' if any work was performed, otherwise it must return 'false'. Returning
// 'true' will cause the idle loop to continue to call this method to process data at the maximum rate (which can peg
// the CPU at 100%.) By returning false when there is no work to do, we are nicer to the system.
bool idleFunc(void *pUserData) {
    ...
    if (nullptr == pInterface) {
        Logger::warn(
            SSTR << "Unable to find interface for update: path[" << objectPath << "], name[" << interfaceName << "]"
        );
    } else {
        // Is it a characteristic?
        if (std::shared_ptr<const GattCharacteristic> pCharacteristic =
                TRY_GET_CONST_INTERFACE_OF_TYPE(pInterface, GattCharacteristic)) {
            Logger::debug(
                SSTR << "Processing updated value for interface '" << interfaceName << "' at path '" << objectPath
                     << "'"
            );
...
//
// A periodic timer is a timer fires every so often (see kPeriodicTimerFrequencySeconds.) This is used for our
// initialization failure retries, but custom code can also be added to a server description (see `onEvent()`)
gboolean onPeriodicTimer(gpointer pUserData) {
    ...
    if (bApplicationRegistered) {
        ...
        //
        // The real goal here is to have the objects tick their interfaces (see `onEvent()` method when adding
        // interfaces inside 'Server::Server()'
...
//
// Our event handlers. These are generic, as they parcel out the work to the appropriate server objects (see
// 'Server::Server()' for the code that manages event handlers.)
// ---------------------------------------------------------------------------------------------------------------------------------
...
void onMethodCall(
    ...
    if (!TheServer
             ->callMethod(objectPath, pInterfaceName, pMethodName, pConnection, pParameters, pInvocation, pUserData)) {
        Logger::error(
            SSTR << " + Method not found: [" << pSender << "]:[" << objectPath << "]:[" << pInterfaceName << "]:["
                 << pMethodName << "]"
        );
        g_dbus_method_invocation_return_dbus_error(
            pInvocation,
            kErrorNotImplemented.c_str(),
...
GVariant *onGetProperty(
    ...
    std::string propertyPath = std::string("[") + pSender + "]:[" + objectPath.toString() + "]:[" + pInterfaceName +
                               "]:[" + pPropertyName + "]";
    if (!pProperty) {
        ...
        g_set_error(
            ...
            G_IO_ERROR,
            G_IO_ERROR_FAILED,
            ("Property(get) not found: " + propertyPath).c_str(),
            pSender
        );
        return nullptr;
    }
    ...
    if (!pProperty->getGetterFunc()) {
        ...
        g_set_error(
            ...
            G_IO_ERROR,
            G_IO_ERROR_FAILED,
            ("Property(get) func not found: " + propertyPath).c_str(),
            pSender
        );
        return nullptr;
    }
    ...
    GVariant *pResult = pProperty->getGetterFunc()(
        pConnection,
        pSender,
        objectPath.c_str(),
        pInterfaceName,
...
gboolean onSetProperty(
    ...
    if (!pProperty) {
        ...
        g_set_error(
            ...
            G_IO_ERROR,
            G_IO_ERROR_FAILED,
            ("Property(set) not found: " + propertyPath).c_str(),
            pSender
        );
        return false;
    }
    ...
    if (!pProperty->getSetterFunc()) {
        ...
        g_set_error(
            ...
            G_IO_ERROR,
            G_IO_ERROR_FAILED,
            ("Property(set) func not found: " + propertyPath).c_str(),
            pSender
        );
        return false;
    }
...
// Convenience method for setting a retry timer so that operations can be continuously retried until we eventually
// succeed
void setRetry() {
    retryTimeStart = time(nullptr);
}

// Convenience method for setting a retry timer so that failures (related to initialization) can be continuously retried
...
void doRegisterApplication() {
    ...
    g_dbus_proxy_call(
        ...
        [](GObject * /*pSourceObject*/, GAsyncResult *pAsyncResult, gpointer /*pUserData*/) {
            ...
            if (nullptr == pVariant) {
                Logger::error(
                    SSTR << "Failed to register application: " << (nullptr == pError ? "Unknown" : pError->message)
                );
                setRetryFailure();
            } else {
...
//           |__/                          |___/
//
// Before we can register our service(s) with BlueZ, we must first register ourselves with D-Bus. The easiest way to do
// this is to use an XML description of our D-Bus objects.
// ---------------------------------------------------------------------------------------------------------------------------------
...
// Configure an adapter to ensure it is setup the way we need. We turn things on that we need and turn everything else
// off (to maximize security.)
//
...
void configureAdapter() {
    ...
    bool scFlag =
        info.currentSettings.isSet(HciAdapter::EHciSecureConnections) == TheServer->getEnableSecureConnection();
    ...
    bool anFlag = (advertisingName.length() == 0 || advertisingName == info.name) &&
    ...
    if (!pwFlag || !leFlag || !brFlag || !scFlag || !bnFlag || !cnFlag || !diFlag || !adFlag || !anFlag) {
        ...
        if (!scFlag) {
            Logger::debug(
                SSTR << (TheServer->getEnableSecureConnection() ? "Enabling" : "Disabling") << " Secure Connections"
            );
        ...
        if (!diFlag) {
            ...
            if (!mgmt.setDiscoverable(TheServer->getEnableDiscoverable() ? 1 : 0, 0)) {
                setRetry();
                return;
            }
        }
        ...
        if (!anFlag) {
            Logger::info(
                SSTR << "Setting advertising name to '" << advertisingName << "' (with short name: '"
                     << advertisingShortName << "')"
            );
...
// Find the BlueZ's GATT Manager interface for the *first* Bluetooth adapter provided by BlueZ. We'll need this to
// register our GATT server with BlueZ.
void findAdapterInterface() {
    ...
    for (guint i = 0; i < g_list_length(pObjects) && bluezGattManagerInterfaceName.empty(); ++i) {
        ...
        pBluezGattManagerProxy =
            reinterpret_cast<GDBusProxy *>(g_dbus_object_get_interface(pBluezAdapterObject, "org.bluez.GattManager1"));
        if (nullptr == pBluezGattManagerProxy) {
            continue;
        }

        // Get the interface proxy for this adapter - this will come in handy later
        pBluezAdapterInterfaceProxy =
            reinterpret_cast<GDBusProxy *>(g_dbus_object_get_interface(pBluezAdapterObject, "org.bluez.Adapter1"));
        if (nullptr == pBluezAdapterInterfaceProxy) {
        ...
        pBluezAdapterPropertiesInterfaceProxy = reinterpret_cast<GDBusProxy *>(
            g_dbus_object_get_interface(pBluezAdapterObject, "org.freedesktop.DBus.Properties")
        );
        if (nullptr == pBluezAdapterPropertiesInterfaceProxy) {
            Logger::warn(
                SSTR << "Failed to get adapter properties proxy for interface 'org.freedesktop.DBus.Properties'"
            );
            continue;
        }
    ...
    pBluezAdapterObject =
        g_dbus_object_manager_get_object(pBluezObjectManager, g_dbus_object_get_object_path(pBluezAdapterObject));
...
//
// An ObjectManager allows us to find out what objects (and from those, interfaces, etc.) are available from an owned
// name. We'll use this to interrogate BlueZ's objects to find an adapter we can use, among other things.
void getBluezObjectManager() {
    g_dbus_object_manager_client_new(
        ...
        [](GObject * /*pSourceObject*/, GAsyncResult *pAsyncResult, gpointer /*pUserData*/) {
            ...
            if (nullptr == pBluezObjectManager) {
                Logger::error(
                    SSTR << "Failed to get an ObjectManager client: "
                         << (nullptr == pError ? "Unknown" : pError->message)
                );
                setRetryFailure();
                return;
            }
...
// Acquire an "owned name" with D-Bus. This name represents our server as a whole, identifying us on D-Bus and allowing
// others (BlueZ) to communicate back to us. All of the D-Bus objects (which represent our BlueZ services,
// characteristics, etc.) will all reside under this owned name.
//
...
void doOwnedNameAcquire() {
    ...
    ownedNameId = g_bus_own_name_on_connection(
        ...
        [](GDBusConnection *, const gchar *, gpointer) {
            ...
            if (0 == periodicTimeoutId) {
                Logger::fatal(
                    SSTR << "Unable to acquire an owned name ('" << TheServer->getOwnedName() << "') on the bus"
                );
                setServerHealth(EFailedInit);
                shutdown();
            } else {
...
void doBusAcquire() {
    ...
    g_bus_get(
        ...
        [](GObject * /*pSourceObject*/, GAsyncResult *pAsyncResult, gpointer /*pUserData*/) {
            ...
            if (nullptr == pBusConnection) {
                Logger::fatal(
                    SSTR << "Failed to get bus connection: " << (nullptr == pError ? "Unknown" : pError->message)
                );
                setServerHealth(EFailedInit);
                shutdown();
            }
...
// Poor-man's state machine, which effectively ensures everything is initialized in order by verifying actual
// initialization state rather than stepping through a set of numeric states. This way, if something fails in an
// out-of-order sort of way, we can still handle it and recover nicely.
void initializationStateProcessor() {
    ...
    //
    // It shouldn't ever happen, but just in case, let's double-check that we're healthy and if not, shutdown
    // immediately
...
void runServerThread() {
    ...
    //
    // In case you're wondering if these really need to be async, the answer is yes. For one, it's the right way to do
    // it. But the more practical response is that the main loop must be running (see below) in order for us to receive
    // and respond to events from BlueZ. Well, one of the calls we need to make during initialization is
    // 'RegisterApplication'. This method will will require that we respond the 'GetNamedObjects' method before it
    // returns. If we were to call the synchronous version of 'RegisterApplication', then we've effectively created a
    // deadlock.
    //
    ...
    //
    // Note that we actually run the idle function from a lambda. This allows us to manage the inter-idle sleep so we
    // don't soak up 100% of our CPU.
