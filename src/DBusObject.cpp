...
// >>
//
// A D-Bus object is a container for any number of functional interfaces to expose on the bus. Objects are referred to
// by their path ("/com/acme/widgets"). Here is a simple example of how D-Bus objects relate to Bluetooth services:
//
...
//
// In English, this would be read as "The Acme company has a widget, which has two characteristics defining the
// manufacturer name and serial number for the widget."
//
...
// service(s). Our Server, however, includes all objects and interfaces, including the GetManagedObjects as well as the
// various interfaces we expose over Bluetooth. Therefore, we'll need a way to know which ones to expose over Bluetooth
// (which is, in general, everything EXCEPT the object containing the org.freedesktop.DBus.ObjectManager interface.)
// Since we manage our objects in a hierarchy, only the root object's publish flag matters.
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//
// We simply add a new child at the given path and add an interface configured as a GATT service to it using the given
// UUID.
...
DBusObject::findInterface(const DBusObjectPath &path, const std::string &interfaceName, const DBusObjectPath &basePath)
    const {
    ...
    for (const DBusObject &child : getChildren()) {
        std::shared_ptr<const DBusInterface> pInterface =
            child.findInterface(path, interfaceName, basePath + getPathNode());
...
std::string DBusObject::generateIntrospectionXML(int depth) const {
    ...
    xml += prefix + "  <annotation name='" + TheServer->getServiceName() + ".DBusObject.path' value='" +
           getPath().toString() + "' />\n";
...
// Emits a signal on the bus from the given path, interface name and signal name, containing a GVariant set of
// parameters
void DBusObject::emitSignal(
    ...
    if (0 == result) {
        Logger::error(
            SSTR << "Failed to emit signal named '" << signalName
