...
#define CHARACTERISTIC_UPDATED_VALUE_CALLBACK_LAMBDA                                                                   \
    [](const GattCharacteristic &self, GDBusConnection *pConnection, void *pUserData) -> bool

#define CHARACTERISTIC_EVENT_CALLBACK_LAMBDA                                                                           \
    [](const GattCharacteristic &self, const TickEvent &event, GDBusConnection *pConnection, void *pUserData)
...
struct GattCharacteristic : GattInterface {
    ...
    //
    // Genreally speaking, these objects should not be constructed directly. Rather, use the `gattCharacteristicBegin()`
    ...
    virtual const std::string getInterfaceType() const {
        return GattCharacteristic::kInterfaceType;
    }
    ...
    // Adds an event to the characteristic and returns a refereence to 'this` to enable method chaining in the server
    // description
    //
    // NOTE: We specifically overload this method in order to accept our custom EventCallback type and transform it into
    // a TickEvent::Callback type. We also return our own type. This simplifies the server description by allowing call
    // to chain.
    ...
    //
    // Note: we specifically override this method in order to translate the generic TickEvent::Callback into our own
    // EventCallback
    ...
    //
    // This method is called by our framework whenever a characteristic's value is updated. If you need to perform any
    // actions when a value is updatd, this is a good place to do that work.
    //
    // If you need to perform the same action(s) when a value is updated from the client (via `onWriteValue`) or from
    // this server, then it may be beneficial to call this method from within your onWriteValue callback to reduce
    // duplicated code. See `callOnUpdatedValue` for more information.
    ...
    //
    // If you need to perform the same action(s) when a value is updated from the client (via onWriteValue) or from this
    // server, then it may be beneficial to place those actions in the `onUpdatedValue` method and call it from from
    // within your `onWriteValue` callback to reduce duplicated code. To call the `onUpdatedValue` method from within
    // your `onWriteValue`, you can use this pattern:
    //
    ...
    GattDescriptor &
    gattDescriptorBegin(const std::string &pathElement, const GattUuid &uuid, const std::vector<const char *> &flags);

    // Sends a change notification to subscribers to this characteristic
    //
    // This is a generalized method that accepts a `GVariant *`. A templated version is available that supports common
    // types called `sendChangeNotificationValue()`.
    //
    // The caller may choose to consult HciAdapter::getInstance().getActiveConnectionCount() in order to determine if
    // there are any active connections before sending a change notification.
    void sendChangeNotificationVariant(GDBusConnection *pBusConnection, GVariant *pNewValue) const;

    // Sends a change notification to subscribers to this characteristic
    //
    // This is a helper method that accepts common types. For custom types, there is a form that accepts a `GVariant *`,
    // called `sendChangeNotificationVariant()`.
    //
    // The caller may choose to consult HciAdapter::getInstance().getActiveConnectionCount() in order to determine if
    // there are any active connections before sending a change notification.
    template <typename T> void sendChangeNotificationValue(GDBusConnection *pBusConnection, T value) const {
