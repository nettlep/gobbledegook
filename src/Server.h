...
// >>
//
// This is the top-level interface for the server. There is only one of these stored in the global `TheServer`. Use this
// object to configure your server's settings (there are surprisingly few of them.) It also contains the full server
...
struct Server {
    ...
    // Returns the set of objects that each represent the root of an object tree describing a group of services we are
    // providing
    ...
    // advertising name.
    const std::string &getAdvertisingName() const {
    ...
    // want, set BOTH advertisingName and advertisingShortName to as empty string ("") to prevent setting the
    // advertising name.
    const std::string &getAdvertisingShortName() const {
        return advertisingShortName;
    }
    ...
    // Our owned name
    //
    // D-Bus uses owned names to locate servers on the bus. Think of this as a namespace within D-Bus. We building this
    // with the server name to keep things simple.
    std::string getOwnedName() const {
    ...
    // advertisingShortName: The short name for this controller, as advertised over LE
    //
    //     According to the spec, the short name is used in case the full name doesn't fit within Extended Inquiry
    //     Response (EIR) or Advertising Data (AD).
    //
    //     IMPORTANT: Setting the advertisingName will change the system-wide name of the device. If that's not what you
    //     want, set BOTH advertisingName and advertisingShortName to as empty string ("") to prevent setting the
    //     advertising name.
    ...
    //
    // If the method was called, this method returns true, otherwise false.  There is no result from the method call
    // itself.
    std::shared_ptr<const DBusInterface>
    findInterface(const DBusObjectPath &objectPath, const std::string &interfaceName) const;
    ...
    const GattProperty *
    findProperty(const DBusObjectPath &objectPath, const std::string &interfaceName, const std::string &propertyName)
        const;
