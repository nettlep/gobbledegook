...
#define DESCRIPTOR_UPDATED_VALUE_CALLBACK_LAMBDA                                                                       \
    [](const GattDescriptor &self, GDBusConnection *pConnection, void *pUserData) -> bool

#define DESCRIPTOR_EVENT_CALLBACK_LAMBDA                                                                               \
    [](const GattDescriptor &self, const TickEvent &event, GDBusConnection *pConnection, void *pUserData)
...
struct GattDescriptor : GattInterface {
    ...
    //
    // Genreally speaking, these objects should not be constructed directly. Rather, use the `gattDescriptorBegin()`
    // method in `GattCharacteristic`.
    ...
    virtual const std::string getInterfaceType() const {
        return GattDescriptor::kInterfaceType;
    }
    ...
    // Adds an event to the descriptor and returns a refereence to 'this` to enable method chaining in the server
    // description
    //
    ...
    //
    // This method is called by our framework whenever a Descriptor's value is updated. If you need to perform any
