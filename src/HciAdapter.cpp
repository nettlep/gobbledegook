...
//
// The Bluetooth Management API, is used to configure the Bluetooth adapter (such as enabling LE, setting the device
// name, etc.) This class uses HciSocket (HciSocket.h) for the raw communications.
//
...
// KNOWN LIMITATIONS:
//
// This is far from a complete implementation. I'm not even sure how reliable of an implementation this is. However, I
// can say with _complete_confidence_ that it works on my machine after numerous minutes of testing.
//
// One notable limitation is that this code doesn't work with the Bluetooth Management API in the way it was intended.
// The Bluetooth Management API treats the sending and receiving of data differently. It receives commands on the HCI
// socket and acts upon them. It also sends events on the same socket. It is important to note that there is not
// necessarily a 1:1 correlation from commands received to events generated. For example, an event can be generated when
// a bluetooth client disconnects, even though no command was sent for which that event is associated.
//
// However, for initialization, it seems to be generally safe to treat them as "nearly 1:1". The solution below is to
// consume all events and look for the event that we're waiting on. This seems to work in my environment (Raspberry Pi)
// fairly well, but please do use this with caution.
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    while (ggkGetServerRunState() <= ERunning && hciSocket.isConnected()) {
        ...
        default: {
            if (eventCode >= kMinEventType && eventCode <= kMaxEventType) {
                Logger::error(
                    "Unsupported response event type: " + Utils::hex(eventCode) + " (" + kEventTypeNames[eventCode] +
                    ")"
                );
            } else {
...
void HciAdapter::stop() {
    ...
    try {
        ...
        if (ex.code() == std::errc::invalid_argument) {
            ...
            Logger::warn(
                SSTR << "Deadlock avoided in call to HciAdapter::wait() (did the event thread try to stop itself?): "
                     << ex.what()
            );
        } else {
            Logger::warn(
                SSTR << "Unknown system_error code (" << ex.code() << ") during HciAdapter::wait(): " << ex.what()
            );
        }
    }
}
...
//
// If the HCI socket is not connected, it will auto-connect prior to sending the command. In the case of a failed
// auto-connect, a failure is returned.
//
...
bool HciAdapter::sendCommand(HciHeader &request) {
    ...
    conditionalValue = -1;
    std::future<bool> fut =
        std::async(std::launch::async, [&]() mutable { return waitForCommandResponse(code, kMaxEventWaitTimeMS); });
...
bool HciAdapter::waitForCommandResponse(uint16_t commandCode, int timeoutMS) {
    ...
    if (!success) {
        Logger::warn(
            SSTR << "  + Timed out waiting on command code " << Utils::hex(commandCode) << " ("
                 << kCommandCodeNames[commandCode] << ")"
        );
    } else {
        Logger::debug(
            SSTR << "  + Recieved the command code we were waiting for: " << Utils::hex(commandCode) << " ("
                 << kCommandCodeNames[commandCode] << ")"
        );
    }
