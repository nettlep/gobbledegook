...
// ---------------------------------------------------------------------------------------------------------------------------------
// Pure virtual representation of a Bluetooth GATT Interface, the base class for Services, Characteristics and
// Descriptors
// ---------------------------------------------------------------------------------------------------------------------------------
...
struct GattInterface : DBusInterface {
    ...
    //
    // There are helper methods for adding properties for common types as well as a generalized helper method for adding
    // a `GattProperty` of a generic GVariant * type.
    template <typename T> T &addProperty(const GattProperty &property) {
    ...
    //
    // There are helper methods for common types (UUIDs, strings, boolean, etc.) Use this method when no helper method
    // exists for the type you want to use. There is also a helper method for adding a named property of a pre-built
    // `GattProperty`.
    template <typename T>
    T &addProperty(
    ...
    template <typename T> T getDataValue(const char *pName, const T defaultValue) const {
    ...
    template <typename T> T getDataPointer(const char *pName, const T defaultValue) const {
        const void *pData = TheServer->getDataGetter()(pName);
        return nullptr == pData ? defaultValue : static_cast<const T>(pData);
    }
    ...
    template <typename T> bool setDataValue(const char *pName, const T value) const {
    ...
    template <typename T> bool setDataPointer(const char *pName, const T pointer) const {
        return TheServer->getDataSetter()(pName, static_cast<const void *>(pointer)) != 0;
    }

    // When responding to a ReadValue method, we need to return a GVariant value in the form "(ay)" (a tuple containing
    // an array of bytes). This method will simplify this slightly by wrapping a GVariant of the type "ay" and wrapping
    // it in a tuple before sending it off as the method response.
    //
    // This is the generalized form that accepts a GVariant *. There is a templated helper method
    // (`methodReturnValue()`) that accepts common types.
    ...
    // it in a tuple before sending it off as the method response.
    //
    // This is a templated helper method that only works with common types. For a more generic form which can be used
    // for custom types, see `methodReturnVariant()'.
    template <typename T>
