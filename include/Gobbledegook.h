...
//     * Logging
//
//       The server defers all logging to the application. The application registers a set of logging delegates (one for
//       each log level) so it can manage the logs however it wants (syslog, console, file, an external logging service,
//       etc.)
//
//     * Managing updates to server data
//
//       The application is required to implement two delegates (`GGKServerDataGetter` and `GGKServerDataSetter`) for
//       sharing data with the server. See standalone.cpp for an example of how this is done.
//
//       In addition, the server provides a thread-safe queue for notifications of data updates to the server.
//       Generally, the only methods an application will need to call are `ggkNofifyUpdatedCharacteristic` and
//       `ggkNofifyUpdatedDescriptor`. The other methods are provided in case an application requies extended
//       functionality.
//
//     * Server control
//
...
//     * Server state
//
//       These routines allow the application to query the server's current state. The server runs through these states
...
// Each of these methods registers a log receiver method. Receivers are set when registered. To unregister a log
// receiver, simply register with `nullptr`.
...
//
// Similarly, the pointer to data returned to the server should point to non-volatile memory so that the server can use
// it safely for an indefinite period of time.
typedef const void *(*GGKServerDataGetter)(const char *pName);

// Type definition for a delegate that the server will use when it needs to notify the host application that data has
// changed
//
// IMPORTANT:
//
// This will be called from the server's thread. Be careful to ensure your implementation is thread safe.
//
// The data setter uses void* types to allow receipt of unknown data types from the server. Ensure that you do not store
// these pointers. Copy the data before returning from your getter delegate.
//
...
//
// `elementLen` is the size of the `element` buffer in bytes. If the resulting string (including the null terminator)
// will not fit within `elementLen` bytes, the method returns `-1` and does nothing.
//
// If `keep` is set to non-zero, the entry is not removed and will be retrieved again on the next call. Otherwise, the
// element is removed.
//
...
//
// If initialization was successful, the method will return a non-zero value with the server running on its own thread
// in 'runServerThread'.
//
// If initialization was unsuccessful, this method will continue to block until the server has stopped. This method will
// then return 0.
//
// IMPORTANT:
//
...
// these pointers. Copy the data before returning from your getter delegate.
//
// Similarly, the pointer to data returned to the data getter should point to non-volatile memory so that the server can
// use it safely for an indefinite period of time.
//
...
//
// The shutdown process will interrupt any currently running asynchronous operation and prevent new operations from
// starting. Once the server has stabilized, its event processing loop is terminated and the server is cleaned up.
//
...
//     EUninitialized -> EInitializing -> ERunning -> EStopping -> EStopped
//
// Note that in some cases, a server may skip one or more states, as is the case of a failed initialization where the
// server will progress from EInitializing directly to EStopped.
//
// Use `ggkGetServerRunState` to retrive the state and `ggkGetServerRunStateString` to convert a `GGKServerRunState`
...
//
// A running server's health will always be EOk, therefore it is only necessary to check the health status after the
// server has shut down to determine if it was shut down due to an unhealthy condition.
//
